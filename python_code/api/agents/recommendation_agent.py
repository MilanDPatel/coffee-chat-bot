...
class RecommendationAgent():
    ...
    def recommendation_classification(self, messages):
        ...
        Your output must be valid JSON in the following format:
        {
            ...
            "recommendation_type": "one of: apriori, popular, or popular by category",
            "parameters": []
        }
        The parameters should be a list of items for apriori or categories for popular by category, or empty for popular.
        """
        ...
        try:
            ...
            # Fallback to popular recommendations if classification fails
            return {
    ...
    def postprocess_classification(self, output):
        try:
            ...
            required_fields = ["recommendation_type", "parameters"]
            ...
            if parsed_output["recommendation_type"] not in valid_types:
                raise ValueError("Invalid recommendation type")
                
            # Validate parameters is a list
            if not isinstance(parsed_output["parameters"], list):
                raise ValueError("Parameters must be a list")

            return {
                "recommendation_type": parsed_output["recommendation_type"],
                "parameters": parsed_output["parameters"]
            }
            
        except (json.JSONDecodeError, ValueError) as e:
            # Default to popular recommendations if there's any error
            return {
                "recommendation_type": "popular",
                "parameters": []
            }
